import type { Priority } from "@/types/_dashboard/kanban"; // Adjust this import path as needed
...
const isValidPriority = (priority: Priority): priority is Priority =>
	["low", "medium", "high"].includes(priority);

const isValidDueDate = (dueDate: string): boolean =>
	/^\d{4}-\d{2}-\d{2}$/.test(dueDate);

const isValidStatus = (status: string): status is string =>
